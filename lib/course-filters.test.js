...
test('filters courses by search and difficulty', () => {
  ...
  const roleResults = filterCourses(courses, { selectedRoles: ['AI Engineer'] });
  assert.equal(roleResults.length, 1);
  assert.equal(roleResults[0].slug, 'copilot-ai');

  const subjectResults = filterCourses(courses, { selectedSubjects: ['Business'] });
  assert.equal(subjectResults.length, 1);
  assert.equal(subjectResults[0].slug, 'power-platform');
});
